...
(``log_cleanup_<timestamp>.log``) rather than piggybacking on another entry point's log.
"""
import traceback
...
from utils.common import get_project_root, load_configs, resolve_log_dir, initialize_run
...
def run_log_cleanup(logger):
    ...
    result = cleanup_old_logs(Path(log_dir), int(retention_days), logger)
    logger.info(
